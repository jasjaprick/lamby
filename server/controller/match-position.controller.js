  ...
  try {
    ...
    if (currentPosition) {
      await db.MatchPosition.destroy({
        where: {
          matchId: matchId, position: position
        }
      })
      await match.addUser(userId, { through: { position, instruction } })
      res.sendStatus(201)
    } else {
      await match.addUser(userId, { through: { position, instruction } })
      res.sendStatus(201)
...
exports.getMatchPositions = async (req, res) => {
  try {
    ...
    const currentMatch = await db.MatchPosition.findAll({ where: { matchId: nextMatch.id } })
    res.status(200)
